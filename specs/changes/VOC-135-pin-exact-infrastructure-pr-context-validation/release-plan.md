...
VOC-135 carrier, so application checks receive an immutable PR base/head pair
...
unrestricted model, a complete eight-path no-change boundary and no
hydration bypass are kept, and exact-revision binding is fail-closed without
a self-referential Git-tree SHA. VOC-129 caller PR #1046 remains the VOC-129
carrier. Exhausted VOC-130-T00 (#1049 / PR #1051), exhausted VOC-131-T00
(PR #1056), unpublished VOC-132-T00 (#1059), exhausted VOC-133-T00
(#1063 / PR #1065), and exhausted VOC-134-T00 (#1068 / PR #1070) are not
retried and are not publishable sources.
...
| T00 caller merge | implementer + independent verifier | Adoption + task authorization; infra #167 already merged as `b263c0c…`; new VOC-135 branch from current `develop`; PR #1051 / #1056 / #1065 / #1070 not reused; VOC-132-T00, VOC-133-T00, and VOC-134-T00 not redispatched; VOC-129 #1046 not rewritten; eight no-change paths identical to the immutable carrier-base SHA; complete-diff scan clean; feasible exact-head evidence | `VOC-135-EV-00` — pin SHA; identify/converge restore coverage; nested-checkout helper-lifetime coverage; PR-context coverage; recorded #167 hashes; complete eight-path boundary vs immutable carrier-base SHA; complete-diff scan; `package.json` identity; preserved #164 contracts; App-authored review/check binds the live PR head; final infra merge |
...
| Exhausted VOC-134 close | maintainers + audit comments | VOC-134-T00 (#1068 / PR #1070) exhausted at heads `f373d9a…` and `718bfdc…`; not redispatched | #1070 closed as superseded (never merged) after this replacement is promoted; #1068 closed with audit links naming the VOC-135 merge; no manufactured VOC-134 marker; VOC-134's adopted pin is not silently rewritten in place |
| Post-merge promotion of this package | existing `release.yml@main` (#167) | T00 caller changes live on `develop` with a valid completion marker | Promotion merge exists; `develop` is advanced to that exact merge SHA before audit close; refs end at the same SHA |
...
| Audit reconciliation | implementer evidence + maintainers | VOC-127 through VOC-135 records reconcile | Release/task/requirement records close with audit comments naming the exact promotion merges and the independently reviewed head; root issue #1071 closes only after that evidence exists |
...
package's pin/test/doc obligation and must not become a second VOC-135 task.

## Rollback

| Item | Value |
|------|-------|
| Trigger | Pin not equal to `b263c0c…`; restore missing in identify or converge; helper copy missing before the unrestricted model; PR-context pair missing or evidence fetch introduced; unique develop commits erased; operator SHA paste; `roles.yml` changed; any of the eight no-change paths rewritten; capture-commit fetch helper, hydrate helper, or provenance-mode wrapper added under any filename; evidence mutated at test time; self-referential exact-head SHA required; PR #1051 / #1056 / #1065 / #1070 reused; VOC-132-T00, VOC-133-T00, or VOC-134-T00 redispatched; `/tmp`-only byte comparison; false-revert evidence; snapshot-gap commit |
| Mechanism | Revert the T00 caller fixture/test/doc changes |
| Owner | Implementer PR + independent verification |
| Validation | Re-run caller governance/fixture suites against the restored `develop` pin `863fc1f35b1d35e4981a59166b0e939be1a2b681`, VOC-112 `subject_revision` `f9d11e23…`, published provenance-test fail-closed `local` mode, and unmodified eight no-change paths |
| Last-known-good | Caller `develop` before this package's merge (pin `863fc1f…`, eight no-change paths unchanged; expected carrier-base SHA `b9e74fc2…`). That last-known-good still lacks the #165 restore, #166 helper-lifetime contract, and #167 PR-context contract in the fixture; rollback is to reviewed state, not to a working #167 pin. Do not revert infrastructure #167. Do not restore #1051's VOC-112 JSON retargets, #1056's provenance-test weakening, #1065's `package.json` / evidence-stamp changes, or #1070's runner fetch / hydrate helpers |
...
1. Exact-SHA independent verification for the caller implementation PR. The
   ...
   `eb11c4fc6841ec73816e2e064dcd449d98c1e933`; 442/442); this package does
   not reopen that review except to confirm the caller consumed that exact
   SHA.
...
9. Deterministic proof that all eight no-change paths are byte-identical to
   the immutable carrier-base SHA, JSON `subject_revision` remains
   ...
   the implementation diff, and the regression fails—not skips—if the exact
   commit or a named path cannot resolve.
10. Deterministic proof that a complete caller-diff scan of changed
    executable paths finds no capture fetch, hydrate/materialize helper,
    provenance-mode override, import side effect, environment override,
    evidence stamp, or local fail-closed bypass under any filename.
11. Confirmation that `t00-evidence.md` names the immutable carrier base and
...
12. Confirmation that current-state fixture README/comments name the #167 pin,
    ...
    VOC-131 / VOC-132 / VOC-133 / VOC-134 records, `AGENTS.md`, the navigator
    skill, the provenance test, the runner, `validate-workspace.mjs`, and
    `package.json` were not rewritten.
13. Explicit record that PR #1051, PR #1056, PR #1065, and PR #1070 are not
    reused, merged, cherry-picked, or modified; that VOC-132-T00 (#1059),
    VOC-133-T00 (#1063), and VOC-134-T00 (#1068) are not redispatched; that
    VOC-129 PR #1046 is not re-implemented; that no VOC-127 through VOC-134
    completion marker was manufactured; that no snapshot-gap task was added;
    and that VOC-127 through VOC-135 close only after valid completion
    markers and successful promotion, not from closed state alone. Root issue
    #1071 closes only after that audit evidence exists.
14. Confirmation that no bootstrap exception was used.
15. After promotion: `develop` and `main` resolve to the same SHA for this
    package's promotion merge; staging ran only for the real tree change;
    tree-equivalent convergence did not trigger an unnecessary staging
    deployment; exhausted / superseded VOC-127 / VOC-130 / VOC-131 / VOC-132 /
    VOC-133 / VOC-134 are closed with audit links naming this replacement.
