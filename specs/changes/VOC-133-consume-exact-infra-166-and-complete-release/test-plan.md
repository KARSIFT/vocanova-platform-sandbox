...
## VOC-133-TEST-00 — Pin equals exact infrastructure #166 merge

- Covers: `VOC-133-AC-00`
...
## VOC-133-TEST-01 — Fixture mirrors the necessary #166 files by recorded SHA-256
...
- Procedure: Assert SHA-256 of fixture `.github/workflows/implement.yml`,
  ...
  recorded from infra merge `f3d791…` (`VOC-133-D11`, reconfirmed at
...
## VOC-133-TEST-02 — Identify restores shared policy before validate-task

- Covers: `VOC-133-AC-01`
...
## VOC-133-TEST-03 — Converge restores shared policy before validate-roster

- Covers: `VOC-133-AC-02`
- Preconditions: mirrored fixture `release.yml`
...
## VOC-133-TEST-04 — Restore uses the immutable reusable-workflow revision without persisting credentials

- Covers: `VOC-133-AC-01`, `VOC-133-AC-02`
- Preconditions: mirrored fixture `release.yml`
...
## VOC-133-TEST-05 — #164 missing-develop, exact-SHA sync, and unique-develop contracts remain

- Covers: `VOC-133-AC-04`
...
## VOC-133-TEST-06 — #166 helper-lifetime and nested-checkout classifier contracts

- Covers: `VOC-133-AC-03`
...
## VOC-133-TEST-07 — Existing recovery, review, retry, and credential contracts remain

- Covers: `VOC-133-AC-05`
...
## VOC-133-TEST-08 — Caller suites, docs, and governance validation
...
- Procedure: Run the commands listed in `implementation-plan.md`. Assert
  ...
  VOC-131/VOC-132 records were not rewritten. Confirm the caller-owned
  fixture README was not replaced by the infrastructure repository README.
  Confirm `AGENTS.md` and `.agents/skills/vocanova-repo-navigator/SKILL.md`
  are not in the implementation diff. Confirm `t00-evidence.md` records the
  exact reviewed head and the final infra merge, that no snapshot-gap task
  was added, no bootstrap exception was used, PR #1051 and PR #1056 were not
  reused, and VOC-132-T00 (#1059) was not redispatched. Confirm
...
## VOC-133-TEST-09 — Complete VOC-112 no-change boundary remains byte-identical to the carrier develop base

- Covers: `VOC-133-AC-06`
...
## VOC-133-TEST-10 — Evidence names the exact reviewed head and does not claim a false revert
...
- Procedure: Assert `t00-evidence.md` records the exact 40-character
  implementation head SHA that independent review binds and the final infra
  merge `f3d79177bf8a9abe0dae550f39502165d494c576`. If evidence claims that a
  protected VOC-112 no-change path was reverted, unchanged, or restored,
  assert that path is absent from `git diff` against the carrier base. The
  VOC-131 class — evidence claiming
  `voc112-navigation-benchmark.test.mjs` was reverted while the tree still
  weakened `local` mode — must fail this test. Assert evidence does not
  contain raw provider responses, credentials, or a full CI log.
- Expected result: evidence truthfully describes the exact reviewed head and
  the consumed infra merge.
- Evidence: `VOC-133-EV-00`

## VOC-133-TEST-11 — Replacement carrier is VOC-133; no snapshot-gap; VOC-129 through VOC-132 are not re-implemented

- Covers: `VOC-133-AC-07`
...
- Procedure: Assert the implementation PR targets `develop` from a new
  VOC-133 branch, `Closes` only its own VOC-133 task issue, and does not
  `Closes` VOC-129, VOC-130, VOC-131, VOC-132, or root #1061. Assert evidence
  records that PR #1051 and PR #1056 were not reused, merged, or modified;
  that VOC-132-T00 (#1059) was not redispatched; that VOC-129 PR #1046 was
  not re-implemented; that no VOC-129, VOC-130, VOC-131, or VOC-132
  ...
  release/sync/closure for VOC-129, VOC-130, VOC-131, VOC-132, and this
  package, with closed state not treated as completion proof.
- Expected result: repair is a new VOC-133 carrier, not a VOC-130 / VOC-131 /
  VOC-132 retry, not a VOC-129 rewrite, and not a snapshot-then-check
  package.
- Evidence: `VOC-133-EV-00`
