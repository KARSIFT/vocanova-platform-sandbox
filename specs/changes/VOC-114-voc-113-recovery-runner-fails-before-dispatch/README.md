...
| Status | `draft` |
| Risk | `R4` (draft proposal; CI/CD lifecycle orchestration and App token contract) |
...
App-backed merge/PR/issue operations succeed; both modes fail at the shared
...
1. Ensure merge-gate, release, and `recover-actions-checks` App tokens carry
   the explicit read capabilities required by exact-SHA metadata endpoints while
   preserving existing narrow mutation permissions.
...
and the KARSIFT App token read contract for exact-SHA gate metadata. The
