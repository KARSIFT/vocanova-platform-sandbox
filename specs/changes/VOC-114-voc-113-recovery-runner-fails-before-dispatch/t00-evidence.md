...
| `.github/workflows/repository-governance.yml` | Keeps original pull requests that modify capture evidence in strict `pr-ancestry` mode, while promotion recovery dispatches validate the already-squashed `develop` history through the existing merge-base-bound `pr-validation` mode |
