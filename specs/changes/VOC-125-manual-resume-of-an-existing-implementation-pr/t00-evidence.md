...
- `.github/workflows/pipeline.yml` — `existing_pr_number` dispatch + forward, delivered by VOC-126 after the original carrier exceeded GitHub's input limit
- `tooling/governance/fixtures/karsift-ai-infra/` — mirrored `implement.yml`, `remediate.yml`, bind helpers, and `test_voc125_existing_carrier.py`
- `tooling/governance/tests/test_voc125_pipeline_dispatch.py`, `test_voc125_implement_fixture.py`, and the consolidated `test_voc125_implement_policy.py`
- caller pin assertions and foundation fixture tests advanced to the later VOC-126 infrastructure merge
- this evidence file
...
| Independently reviewed infra head SHA | `f2b69b45226305f7ee820fdb834929098bb5c232` — `PASS WITH NON-BLOCKING FINDINGS` after remediation |
| Exact infra git merge SHA | `1f1705dbad41729563b0ad1e878e4154e5511e93` |
| Later source merge consumed by replacement caller | KARSIFT/karsift-ai-infra#161 merge `20dcf340fa73a36ebc6074442fde79530dfa5871`, containing the VOC-125 source tree plus the VOC-126 input-limit repair |
| Pin applicable? | **yes** — `implement.yml`, `remediate.yml`, bind helpers, and tests consumed |
| `PINNED_SHA.txt` on the replacement caller | `20dcf340fa73a36ebc6074442fde79530dfa5871` — exact later infrastructure merge SHA |
| Bootstrap used? | **no** |
...
## VOC-126 delivery reconciliation

Caller PR #1024 at `8621f12dd466edab37fddb86d4e5e0a348ed3609`
was not merged because GitHub rejected its 26-input workflow definition before
jobs could start. Adopted VOC-126-T00 carries this VOC-125 implementation and
its caller-specific regression tests into replacement caller PR #1029, together
with the split `pipeline.yml` / `pipeline-verify.yml` dispatch contract. The
combined fixture pins infrastructure merge
`20dcf340fa73a36ebc6074442fde79530dfa5871`, which contains the earlier VOC-125
source merge. This section records delivery reconciliation only; #1024 remains
unmerged, and VOC-125 issues #1022/#1020 remain open until #1029 is reviewed,
merged, and promoted.
...
| Command | Result |
|---------|--------|
| `python3 -m unittest discover -s tests -p 'test_*.py'` (karsift-ai-infra) | **OK** (386 tests on source head `9db35bd338313ab3ab8b7903a6c0522383423fa4`) |
| `python3 -m unittest discover -s tooling/governance/fixtures/karsift-ai-infra/tests -p 'test_*.py'` | **OK** (343 tests on the final composed tree) |
| `bash scripts/governance/validate-governance.sh` | **OK** |
| `bash scripts/governance/classify-change-risk.sh` | **OK** (R4 path floor) |
| `python3 -m unittest discover -s tooling/governance/tests -p 'test_*.py'` | **OK** (208 tests on the final composed tree) |
| `git diff --check` | **OK** |
...
- `VOC-125-AC-05` / `VOC-125-EV-00` — preserved; existing VOC-121/123/124 policy tests remain part of validation
- `VOC-125-AC-06` / `VOC-125-EV-00` — docs/comments updated; exact pin and #1003/#1012 handoff recorded above
