...
| Remote probe explicitly confirms proof file absent (`rotate_host` not success)                  | SKIP_UNUSED (do not overwrite secret)                                                               | SCRUB unused copies                                            |
| Host reachable but proof presence/transfer is inconclusive                                      | RETAIN (fail closed)                                                                                | RETAIN last copies                                             |
...
2. Scrubs only after both secret writes complete, or when a reserved remote
   probe result explicitly proves this attempt's reset marker is absent;
   reachability and transfer failure are never treated as absence.
