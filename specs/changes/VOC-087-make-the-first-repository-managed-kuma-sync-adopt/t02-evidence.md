...
| --- | --- | --- |
| Reset-applied proof matches this attempt | STORE from runner-local file | SCRUB after `password_stored=true` |
| `rotate_host` succeeded, proof/metadata fetch failed | STORE from runner-local file | RETAIN until store succeeds, then SCRUB |
...
| `recover_store_only` | store host-retained file; never calls `reset-password.js` | SCRUB only after that store |
...
1. Treats `password_stored` (not step success) as the store-complete signal.
2. Scrubs only after a completed store, or when a successful host fetch proves
   this attempt did not reset Kuma, or when rotate was skipped.
3. Adds `recover_store_only` as a later store-only retry that never resets.
4. Invokes host rotation once; recover-store cannot be combined with rotate.
