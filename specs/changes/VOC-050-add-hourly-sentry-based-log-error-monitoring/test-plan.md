...
## VOC-050-TEST-00 — `apps/web` sends a test error to Sentry when a DSN is configured
...
## VOC-050-TEST-01 — `apps/web` does not error, warn noisily, or expose a debug overlay when no DSN is configured, and the debug overlay is disabled in production/staging builds regardless
...
## VOC-050-TEST-02 — The Sentry API auth token's configured scope is read-only and least-privilege

- Covers: `VOC-050-AC-01`
- Preconditions: the token's scope as configured in Sentry's own token
  management UI/API, and `VOC-050-T02`'s workflow-file documentation of the
  expected scope
...
## VOC-050-TEST-03 — The scheduled workflow queries both the staging and production Sentry environments
...
## VOC-050-TEST-04 — A genuinely new Sentry problem produces exactly one unlabeled GitHub issue with sufficient reproduction/diagnosis detail
...
## VOC-050-TEST-05 — Repeated runs against the same underlying Sentry issue create no second GitHub issue

- Covers: `VOC-050-AC-04`
- Preconditions: same seeded Sentry issue as `VOC-050-TEST-04`, already
...
## VOC-050-TEST-06 — The scheduled workflow file declares no SSH-related secret
...
## VOC-050-TEST-07 — DOC-11 §1's amendment accurately and non-destructively records the new mechanism

- Covers: `VOC-050-AC-06`
- Preconditions: `VOC-050-T04`'s merged documentation change
- Procedure: read the amended `docs/operations/11-devops-and-ci-cd.md` §1;
  ...
  accurately describes what `VOC-050-T01`-`T03` actually built (not what was
...
## VOC-050-TEST-08 — Deterministic validation commands pass against the exact implemented diff

- Covers: `VOC-050-AC-07`
...
## VOC-050-TEST-09 — Sentry organization/plan capacity is confirmed before dependent tasks proceed

- Covers: (gates `VOC-050-AC-00`, `VOC-050-AC-01` via `VOC-050-T00`)
