...
## VOC-050-T00 — Confirm Sentry organization/plan capacity and record the exact per-environment DSN/project layout before any code change

- Requirement source: `specification.md`'s open questions 3; `change.yaml`'s
  `VOC-050-DEP-00`
- Acceptance criteria: (gates `VOC-050-AC-00`, `VOC-050-AC-01`)
- Tests: `VOC-050-TEST-09`
...
`VOC-050-T01`/`VOC-050-T02` proceed with a revised scope.

## VOC-050-T01 — Wire the Sentry SDK into `apps/web`, mirroring `apps/api`'s existing pattern

- Requirement source: `specification.md`'s scope item 1
- Acceptance criteria: `VOC-050-AC-00`, `VOC-050-AC-07`
- Tests: `VOC-050-TEST-00`, `VOC-050-TEST-01`, `VOC-050-TEST-08`
- Evidence: `VOC-050-EV-00`, `VOC-050-EV-03`
- Status: pending (depends on `VOC-050-T00`'s confirmed layout)
...
## VOC-050-T02 — Add the hourly scheduled workflow that queries Sentry and files deduplicated GitHub issues

- Requirement source: `specification.md`'s scope items 2-4; open questions 1, 2, 4
- Acceptance criteria: `VOC-050-AC-01`, `VOC-050-AC-02`, `VOC-050-AC-03`,
  `VOC-050-AC-04`, `VOC-050-AC-05`, `VOC-050-AC-07`
- Tests: `VOC-050-TEST-02`, `VOC-050-TEST-03`, `VOC-050-TEST-04`,
  `VOC-050-TEST-05`, `VOC-050-TEST-06`, `VOC-050-TEST-08`
- Evidence: `VOC-050-EV-01`, `VOC-050-EV-03`
- Status: pending (depends on `VOC-050-T00`'s confirmed layout; may proceed in
  parallel with `VOC-050-T01` once `T00` completes, since the workflow's
...
- Use the new Sentry API auth token (`VOC-050-DEP-02`'s scope, confirmed by
  `VOC-050-T00`/this task) to query for new/unresolved issues across both the
...
- Before creating any GitHub issue, search existing open issues for a match
  ...
  `VOC-050-R01`), following the same idempotency shape as
...
- Declare and use no SSH-related secret of any kind (`VOC-050-AC-05`).
- Fail the workflow run (non-zero exit) on a genuine Sentry API error,
  distinct from a legitimate zero-new-issues result, per
  `impact-analysis.md`'s `VOC-050-R02`.

## VOC-050-T03 — Add the Sentry API auth token as a GitHub Actions secret

- Requirement source: `specification.md`'s scope item 2
- Acceptance criteria: `VOC-050-AC-01`
- Tests: `VOC-050-TEST-02`
- Evidence: `VOC-050-EV-01`
- Status: pending (depends on `VOC-050-T00`'s confirmed layout; this task's
  ...
  `VOC-050-T02`'s workflow file and this package's own README/impact
...
`VOC-050-T02` workflow expects (proposed: `SENTRY_API_AUTH_TOKEN`) and its
...
workflow can run successfully. If the secret is absent, `VOC-050-T02`'s
...
## VOC-050-T04 — Update DOC-11 §1's "Error monitoring" row to record the new mechanism

- Requirement source: `AGENTS.md`'s documentation-consistency rule
- Acceptance criteria: `VOC-050-AC-06`, `VOC-050-AC-07`
- Tests: `VOC-050-TEST-07`, `VOC-050-TEST-08`
- Evidence: `VOC-050-EV-02`, `VOC-050-EV-03`
- Status: pending (depends on `VOC-050-T01`-`VOC-050-T03` landing, or may land
  in the same pull request as `VOC-050-T02` if the implementer prefers a
...
e.g. `VOC-050-§1-amendment`, in the same style as the existing
