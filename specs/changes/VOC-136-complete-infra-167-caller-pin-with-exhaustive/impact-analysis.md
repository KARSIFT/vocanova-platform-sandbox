...
no-change boundary against protected comparison anchor `b9e74fc2…`, forbids
relocated hydration/fetch/provenance bypasses via an exhaustive caller-diff
scan that includes `tooling/governance/tests/**`, and represents exact-head
binding as App-authored review/check metadata. It does not introduce new
secret values, OAuth/session material, production-data access, or user-facing
data flows. It does not rotate `KARSIFT_BOT_APP_ID` /
...
- Raw helper errors remain sanitized. No credential values are printed in
  ...
  provider responses from PR #1070 or PR #1075.
...
- VOC-112 capture fixtures, provenance test, runner, `validate-workspace.mjs`,
  ...
  bytes and are not recaptured, wrapped, hydrated, or weakened onto an
  implementation head.
...
classifier, #167 immutable PR-context contract, exhaustive executable bypass
scan, and feasible exact-revision evidence contract are CI orchestration
only.

## Risks, dependencies, and evidence

- `VOC-136-R00`: **High correctness risk** if the caller pin remains
  `863fc1f…` or is set to `8ce2b77…` or `f3d791…` while live
  `implement.yml@main` / `release.yml@main` / `ci.yml@main` are already #167.
  Local suites would then assert a stale fixture. Mitigation: `VOC-136-D02`,
  `VOC-136-AC-00`.
- `VOC-136-R01`: **High release-safety risk** if `identify` restores shared
  policy but `converge` does not. Mitigation: `VOC-136-D03`, `VOC-136-AC-02`.
- `VOC-136-R02`: **High correctness risk** if restore uses a mutable ref
  instead of `job.workflow_sha`, or persists credentials. Mitigation:
  `VOC-136-D03`, `VOC-136-TEST-04`.
- `VOC-136-R03`: **High publication-safety risk** if helpers are still copied
  after the unrestricted model. Mitigation: `VOC-136-D05`, `VOC-136-AC-03`,
  `VOC-136-TEST-06`.
- `VOC-136-R04`: **High correctness risk** if a surviving nested path that
  inherits caller Git, is a symlink, or is a non-directory is treated as
  `absent` and discarded. Mitigation: `VOC-136-D05`, `VOC-136-TEST-06`.
- `VOC-136-R05`: **High data-loss risk** if pinning #167 accidentally drops
  #164 unique-develop fail-closed sync or missing-`develop` recovery.
  Mitigation: `VOC-136-D04`, `VOC-136-AC-04`.
- `VOC-136-R06`: **High provenance-safety risk** if `run-app-checks.sh`, CI,
  ...
  Mitigation: `VOC-136-D06`, `VOC-136-AC-05`, `VOC-136-TEST-07`.
- `VOC-136-R07`: **High provenance-safety risk** if application checks fetch
  missing VOC-112 evidence commits. Mitigation: `VOC-136-D06`,
  `VOC-136-TEST-07`.
- `VOC-136-R08`: **High delivery risk** if this package snapshots the current
  develop/main gap (`karsift-ai-infra#15`), reuses PR #1051, PR #1056,
  PR #1065, PR #1070, or PR #1075, redispatches VOC-132-T00 (#1059),
  VOC-133-T00 (#1063), VOC-134-T00 (#1068), or VOC-135-T00 (#1073), or
  re-implements VOC-127 through VOC-135. Mitigation: `VOC-136-D01`,
  `VOC-136-D10`, `VOC-136-AC-10`.
- `VOC-136-R09`: **High correctness / review-safety risk** if the
  ...
  `VOC-136-D14`, `VOC-136-AC-07`, `VOC-136-TEST-10`.
- `VOC-136-R10`: **High correctness / review-safety risk** if the
  ...
  `VOC-136-D14`, `VOC-136-AC-07`.
- `VOC-136-R11`: **High coverage risk** if the identity regression again
  ...
  navigator skill editable. Mitigation: `VOC-136-D14`, `VOC-136-TEST-10`.
- `VOC-136-R12`: **High correctness risk** if #167 byte identity depends on a
  machine-specific `/tmp` checkout that CI or another machine does not have.
  Mitigation: `VOC-136-D11`, `VOC-136-TEST-01`.
- `VOC-136-R13`: **High review-safety risk** if `t00-evidence.md` claims a
  ...
  as on #1065. Mitigation: `VOC-136-D12`, `VOC-136-TEST-13`.
- `VOC-136-R14`: **High review-safety risk** if committed evidence is required
  to contain the SHA of the same commit that contains it. Mitigation:
  `VOC-136-D12`, `VOC-136-AC-09`.
- `VOC-136-R15`: **High correctness / review-safety risk** if `package.json`
  is edited to fetch a missing capture commit or to force
  `VOC112_CAPTURE_PROVENANCE_MODE`, repeating VOC-133 attempts 1 and 2.
  Mitigation: `VOC-136-D14`, `VOC-136-AC-08`, `VOC-136-TEST-11`.
- `VOC-136-R16`: **High correctness / review-safety risk** if an import-time
  ...
  including a `tooling/governance/tests/*.py` module. Mitigation:
  `VOC-136-D14`, `VOC-136-D16`, `VOC-136-AC-08`, `VOC-136-TEST-12`,
  `VOC-136-TEST-15`.
- `VOC-136-R17`: **High correctness risk** if the no-change regression uses a
  moving `develop` / `origin/develop` ref after the protected comparison
  anchor is selected, or if implementation proceeds against
  unrelated/material movement of `develop`. Mitigation: `VOC-136-D15`,
  `VOC-136-TEST-10`, `VOC-136-TEST-17`.
- `VOC-136-R18`: **High coverage / review-safety risk** if the complete-diff
  scan excludes `tooling/governance/tests/**` or this regression's own
  module, repeating VOC-135 attempt 2, or if treating PR #1075 PASS WITH
  NON-BLOCKING FINDINGS as sufficient leaves that exclusion in place.
  Mitigation: `VOC-136-D16`, `VOC-136-AC-08`, `VOC-136-AC-11`,
  `VOC-136-TEST-12`.
- `VOC-136-R19`: **High coverage risk** if the complete-diff scan matches its
  own assertion literals after the module is tracked, repeating VOC-135
  attempt 1, or if a pass obtained only while untracked is treated as
  acceptance. Mitigation: `VOC-136-D16`, `VOC-136-TEST-16`.
- `VOC-136-R20`: **Medium coverage risk** if caller tests advance the pin but
  do not assert restore-before-helper ordering, helper-copy-before-model
  ordering, or PR-context pair passing. Mitigation: `VOC-136-D07`,
  `VOC-136-TEST-02`, `VOC-136-TEST-03`, `VOC-136-TEST-06`, `VOC-136-TEST-07`.
- `VOC-136-R21`: **Low application-runtime release risk** because ordinary
...
- Protected surfaces: caller `tooling/governance/` fixtures and tests,
  possibly live `.github/workflows/*` if `VOC-136-DEP-07` proves a caller
  dispatch file must change, current-state fixture README, and this package
  directory. The eight no-change paths are protected against change relative
  to the protected comparison anchor.
- `VOC-136-DEP-00` through `VOC-136-DEP-13`: see `change.yaml`.
- `VOC-136-EV-00`: T00 evidence — pin SHA, restore coverage, nested-checkout
  coverage, PR-context coverage, complete eight-path no-change boundary
  against the protected comparison anchor, implementation PR base,
  exhaustive-diff hydration/bypass scan including
  `tooling/governance/tests/**`, negative cases, `package.json` identity,
  hash-based #167 identity, feasible exact-revision evidence, preserved #164
  contracts, validation commands after commit, and VOC-127 through VOC-136
  promotion handoff.
