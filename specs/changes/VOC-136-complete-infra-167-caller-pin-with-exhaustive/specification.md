# VOC-136 — Complete infra #167 caller pin with exhaustive executable bypass scanning: Specification

## Objective and requirement source

Deliver the caller-side pin, fixture, tests, docs, complete no-hydration
boundary, exhaustive executable bypass scan, and feasible exact-revision
...
named no-change paths byte-identical to protected comparison anchor
`b9e74fc2db4691c48c637639b265d527de9f4505`, pass an exhaustive complete-diff
scan that includes `tooling/governance/tests/**` and this regression's own
module, pass independent exact-revision review and all protected checks
without self-referential commit SHAs or provenance hydration bypasses, and
then complete ordinary develop-to-main promotion for this package and cleanup
of superseded VOC-127 / VOC-130 through VOC-135 carriers.
...
at `429d8c6d49303148ca1cc14dba5f6768a7863346`. VOC-130 through VOC-135
adopted overlapping #165/#166/#167 pin outcomes. VOC-135-T00
([#1073](https://github.com/KARSIFT/vocanova-platform-sandbox/issues/1073) /
PR [#1075](https://github.com/KARSIFT/vocanova-platform-sandbox/pull/1075))
exhausted both attempts on a self-scanning false-positive and a wholesale
`tooling/governance/tests/` scan exclusion. This package is the governed
replacement carrier. It is not a second VOC-129 implementation, not a retry
of VOC-130 through VOC-135, and not a silent retarget of VOC-135's exhausted
#167 pin.

**This draft package does not adopt or authorize itself**; adoption remains a
separate A-004-governed decision.

### Confirmed problem evidence (issue #1076)
...
| Adopted VOC-135 plan | [#1072](https://github.com/KARSIFT/vocanova-platform-sandbox/pull/1072) |
| Exhausted VOC-135 task | [#1073](https://github.com/KARSIFT/vocanova-platform-sandbox/issues/1073) (`VOC-135-T00`), closed not-planned with audit; no completion marker |
| Exhausted VOC-135 root | [#1071](https://github.com/KARSIFT/vocanova-platform-sandbox/issues/1071), closed not-planned with audit |
| Exhausted VOC-135 carrier | [#1075](https://github.com/KARSIFT/vocanova-platform-sandbox/pull/1075), closed/unmerged; remote branch deleted |
| VOC-135 attempt-1 FAIL head | `7c3e821bb3228c0fd2c7279b6d577a388148cb18` |
| VOC-135 attempt-1 defect | complete-diff scan matched its own assertion literal; 240 tests / 1 failure; independent FAIL [comment](https://github.com/KARSIFT/vocanova-platform-sandbox/pull/1075#issuecomment-5444183121) |
| VOC-135 attempt-2 head | `dcd2da9ccbdf471c5294db30f09c9bb99aacdcd1` |
| VOC-135 attempt-2 defect | added `"tooling/governance/tests/"` to `SCAN_EXCLUDE_PREFIXES`; supervisor [comment](https://github.com/KARSIFT/vocanova-platform-sandbox/pull/1075#issuecomment-5444311947) |
| VOC-135 attempt-2 review | PASS WITH NON-BLOCKING FINDINGS [comment](https://github.com/KARSIFT/vocanova-platform-sandbox/pull/1075#issuecomment-5444342914); **not sufficient** for a replacement that retains the exclusion |
...
3. Add deterministic caller regressions for (a) exact pin consistency with
   ...
   no-change paths against the protected comparison anchor, (g) an exhaustive
   caller-diff scan of changed executable paths including
   `tooling/governance/tests/**` and this regression's own module, with
   source-safe literals, required negative unit cases, benign-mention
   non-false-positives, and a tracked-after-commit pass, and (h) feasible
   exact-revision evidence that does not require a commit to contain its own
   SHA.
4. Preserve VOC-113 through VOC-135 fail-closed contracts already on the
...
6. After the exact reviewed caller pin is merged, complete ordinary
   ...
   VOC-130 through VOC-135, and this package. This package's implementation
   PR `Closes` only its own VOC-136 task issue. Root issue #1076 closes only
   after that promotion/audit evidence exists. Clean only obsolete remote
   automation branches/PRs/issues; preserve unrelated VOC-128 and all user
   worktrees.
...
- Silently changing VOC-132's adopted exact pin from #165, VOC-133 /
  VOC-134's adopted pin from #166, or VOC-135's adopted pin from #167, in
  place.
- Redispatching VOC-132-T00 (#1059), VOC-133-T00 (#1063), VOC-134-T00
  (#1068), or VOC-135-T00 (#1073), resetting any attempt counter, or treating
  this package as another VOC-132 / VOC-133 / VOC-134 / VOC-135
  implementation attempt.
- Reusing, merging, cherry-picking, modifying, or composing PR #1051,
  PR #1056, PR #1065, PR #1070, PR #1075, or their branches.
- Re-implementing VOC-129, re-merging PR #1046, or manufacturing a VOC-127,
  VOC-129, VOC-130, VOC-131, VOC-132, VOC-133, VOC-134, or VOC-135 completion
  marker.
...
- Treating PR #1075 attempt-2 PASS WITH NON-BLOCKING FINDINGS as sufficient
  for a replacement that retains that exclusion.
- Requiring a tracked file to contain the SHA of the same commit that
  contains it.
- Using a moving branch ref (`develop`, `origin/develop`) as the eight-path
  comparison authority after the protected comparison anchor is selected.
- Implementing against unrelated/material movement of `develop`. Issue #1076
  requires fail-closed behavior for that class. This package's own
  plan/adoption/roster commits after `b9e74fc2…` are governance-only and do
  not count as that class.
...
- Rewriting historical CHANGELOG, A-003, VOC-075, VOC-127, VOC-129, VOC-130,
  VOC-131, VOC-132, VOC-133, VOC-134, or VOC-135 audit records except for a
  new current-state note where a live contract would otherwise stay false.
  The fixture copy of infrastructure `CHANGELOG.md` may be replaced
  byte-for-byte from merge `b263c0c…`; that is a fixture mirror, not a
  rewrite of caller audit packages.
...
- Operator-owned live-evidence contracts: acceptance is deterministic tests
  plus exact-SHA review. VOC-127 through VOC-136 promotion/closure are
...
- Protected areas: caller `tooling/governance/` fixtures and tests; live
  ...
  appear in the implementation diff against the protected comparison anchor.
- Protected technical effect: whether the caller fixture and pin equal the
  ...
  `develop` bytes at the protected comparison anchor; whether the
  complete-diff scan covers every added caller executable including
  `tooling/governance/tests/**`; whether exact-revision binding remains
  fail-closed without a self-referential Git-tree SHA. No application runtime
  effect is intended for ordinary promotions.
- EHR: not triggered.
...
`VOC-136-D00`: This is one outcome-sized caller replacement. Use one
...
PR-context validation, the complete eight-path no-change boundary, exhaustive
caller-diff executable bypass scan, `package.json` identity, hash-based #167
...
split reasons. VOC-129 and superseded VOC-127 / VOC-130 through VOC-135
...
`VOC-136-D01`: Consume already-merged infrastructure #167. Do not compose a
replacement infra PR. Do not silently retarget VOC-132 from pin #165,
VOC-133 / VOC-134 from pin #166, or VOC-135 from pin #167. Do not redispatch
VOC-132-T00 (#1059), VOC-133-T00 (#1063), VOC-134-T00 (#1068), or VOC-135-T00
(#1073). Do not reuse, merge, cherry-pick, modify, or compose PR #1051,
PR #1056, PR #1065, PR #1070, or PR #1075 or their branches. Do not treat the
untracked local `karsift-ai-infra/` checkout (if present) as this
repository's tracked tree.

`VOC-136-D02`: Pin and fixture identity is exact merge
...
`VOC-136-D03`: Both `identify` and `converge` must restore shared lifecycle
...
`VOC-136-D04`: Preserve the already-approved #164 behavioral contract in the
...
`VOC-136-D05`: Preserve #166 post-implementer nested-checkout behavior:
...
`VOC-136-D06`: Consume the #167 immutable PR-context validation contract
...
`VOC-136-D07`: Deterministic tests must prove:
...
15. all eight no-change paths are byte-identical to the protected comparison
    anchor and do not appear in `git diff` against that SHA;
...
17. no capture-commit fetch helper, hydrate/materialize helper,
    ...
    under any filename, proven by an exhaustive caller-diff scan of changed
    executable paths including `tooling/governance/tests/**` and this
    regression's own module, rather than a single prohibited filename or a
    wholesale directory exclusion;
18. `t00-evidence.md` records the protected comparison anchor, the actual
    implementation PR base, the final infra merge, confirmed hashes,
    ...
    contain its own SHA;
19. this package's caller PR `Closes` only its own VOC-136 task issue.
...
`VOC-136-D08`: Preserve VOC-113 through VOC-135 fail-closed contracts: roster
...
controls. This package's caller PR `Closes` only its own VOC-136 task issue.

`VOC-136-D09`: Current-state comments in fixture `implement.yml` /
...
`VOC-136-D10`: VOC-129 caller PR #1046 remains the VOC-129 carrier. Exhausted
VOC-130-T00 (#1049 / PR #1051), exhausted VOC-131-T00 (PR #1056), unpublished
VOC-132-T00 (#1059), exhausted VOC-133-T00 (#1063 / PR #1065), exhausted
VOC-134-T00 (#1068 / PR #1070), and exhausted VOC-135-T00 (#1073 / PR #1075)
are not retried. Do not manufacture a VOC-127, VOC-129, VOC-130, VOC-131,
VOC-132, VOC-133, VOC-134, or VOC-135 completion marker. After the repaired
`implement.yml@main` / `release.yml@main` / `ci.yml@main` path is live and
...
3. audit comments name the exact promotion merges and close VOC-127,
   VOC-130, VOC-131, VOC-132, VOC-133, VOC-134, VOC-135, and this
   replacement, then root issue #1076, with precise links. Do not snapshot
   the current develop/main gap. Do not treat closed state alone as
   completion proof.
4. Clean only obsolete remote automation branches/PRs/issues. Preserve
   unrelated VOC-128 and all user worktrees.

`VOC-136-D11`: Exact mirrored fixture bytes are proven by recorded SHA-256
...
Drafting-time hashes are the same values recorded in VOC-135-D11 from an
untracked nested checkout whose `main` resolved to
`b263c0c110591cc798b89277dfc35542abb1597b`, reused here because #167 has not
moved:
...
`VOC-136-D12`: Feasible exact-revision evidence. Exact reviewed-revision
...
1. the protected comparison anchor
   `b9e74fc2db4691c48c637639b265d527de9f4505`;
2. the actual implementation PR base recorded before any in-scope edit;
3. the exact infra merge `b263c0c110591cc798b89277dfc35542abb1597b`;
4. the reconfirmed SHA-256 hashes;
5. complete-diff scan scope, including that `tooling/governance/tests/**`
   was scanned, and the negative-case results;
6. the validation commands actually run, after the regression is tracked and
   committed;
7. the contract that the live implementation head is bound by the
...
8. where that later exact-head binding will be published (the independent
...
revert unless `git diff` against the protected comparison anchor does not
name that path. The VOC-131 false-revert class, the VOC-133 evidence-stamp
class, the VOC-134 hydrate-helper class, and the VOC-135 scan-exclusion
class are blocking defects if they recur. Evidence must not copy raw
provider responses, credentials, or the full log from PR #1070 or PR #1075.

`VOC-136-D13`: No bootstrap exception. VOC-124 already requested
`permission-workflows: write` on `publish-source`. T00's first run is attempt
`1` on a new VOC-136 carrier from current `develop`. Do not treat an
...
PR #1075, VOC-132-T00 (#1059), VOC-133-T00 (#1063), VOC-134-T00 (#1068), or
VOC-135-T00 (#1073) as a publishable or composable source.

`VOC-136-D14`: Eight-path no-change boundary and provenance-bypass
prohibition. All of the following must remain byte-for-byte identical to
protected comparison anchor `b9e74fc2db4691c48c637639b265d527de9f4505` and
must be absent from the implementation PR diff against that SHA:
...
`VOC-136-D15`: Protected comparison anchor versus implementation PR base.
Protected comparison anchor is immutable
`b9e74fc2db4691c48c637639b265d527de9f4505`. Plan/adoption/roster commits
after that SHA — including already-merged VOC-135 plan adoption and this
package's own plan/adoption/roster commits — are governance-only and do not
count as protected-file drift for the eight no-change paths. Issue-creation
`develop` `5044d62f6f35069412e8ffcf80a0682e7847d1c1` is expected to differ
from the anchor for that reason.

Implementation must resolve current `develop` to a 40-character SHA **before
any in-scope edit** and record that SHA as the implementation PR base at PR
creation. Fail closed on unrelated/material movement of `develop`: any tree
change outside this package directory, already-merged VOC-135 package
adoption/roster metadata, and other governance-only commits that cannot
affect the eight protected paths or the live pin/fixture. If any of the
eight no-change paths differs from the protected comparison anchor, fail
closed. If the live pin or fixture already changed, fail closed rather than
composing with an unexpected tree. Do not retarget the protected comparison
anchor. After the implementation PR base is recorded, tests must not use a
moving branch ref (`develop`, `origin/develop`) as the eight-path comparison
authority. The eight-path regression must require that exact protected
comparison-anchor commit object and all eight no-change paths to resolve
(`git cat-file` / `git show SHA:path` or equivalent) and must fail—not
skip—if the commit or any named path cannot be read.

This decision replaces VOC-135-D15's "fail closed if `develop` moved at
all" rule. Issue #1076 requires the distinction because this package cannot
be implemented from `b9e74fc2…` as HEAD once its own plan has merged.

`VOC-136-D16`: Exhaustive caller-diff executable bypass scan. Enumerate
every added or modified path against the protected comparison anchor. Scan:

- all `scripts/**`;
- `package.json`;
- every added/changed `*.mjs`, `*.js`, `*.sh`, and `*.py` anywhere in the
  caller tree except the mirrored
  `tooling/governance/fixtures/karsift-ai-infra/**` subtree;
- any other newly executable caller file.

Do **not** exclude `tooling/governance/tests/**`, this regression's own
module, or another executable directory wholesale. A
`SCAN_EXCLUDE_PREFIXES` (or equivalent) entry for
`tooling/governance/tests/` is a blocking defect — that is the VOC-135
attempt-2 class. Those modules execute during hosted unittest discovery and
could perform an import-time capture fetch, evidence hydration/
materialization, or provenance-environment override before later checks.

The scan must fail if any newly added caller script or any changed existing
caller script:
...
4. adds an import-time or pre-test side effect that fetches or stamps
   evidence, including inside an added `tooling/governance/tests/*.py`
   module;
5. skips or otherwise makes default `local` fail-closed unreachable.

Test/assertion literals that would otherwise match the scanner must be
represented as non-contiguous source-safe values so self-scanning a tracked
committed module does not false-positive (the VOC-135 attempt-1 class).
Actual semantic setters and commands reconstructed in memory as contiguous
payloads must fail. Negative unit cases must prove the scanner rejects at
least:

- an import-time or pre-test `git fetch` for VOC-112/evidence;
- a hydrate/materialize helper under an arbitrary new filename;
...
- relevant `PR_BASE_SHA` / `PR_HEAD_SHA` setters around tests or
  `validate-workspace`;
- an import-time side effect inside an added `tooling/governance/tests/*.py`
  module;
- a wrapper or skip that makes default local fail-closed unreachable.

Those contiguous forbidden payloads may be reconstructed only in memory (or
in non-executable, non-scanned test data). They must not be committed as
contiguous source in an executable file that the scan would then
false-positive on.

Also prove benign assertion/test-data mentions do not false-positive. The
regression itself must pass after it is tracked and committed, not merely
while untracked.

Fixture copies of infra `run-app-checks.sh` that legitimately export
provenance for an immutable PR pair are in-scope mirrors of #167, not
caller-side overrides, and are not this scan's failure class.

Do not treat PR #1075 attempt-2 PASS WITH NON-BLOCKING FINDINGS as
sufficient for a replacement that retains the `tooling/governance/tests/`
exclusion.

`VOC-136-D17`: VOC-135 is exhausted. Reimplementation may follow the
authoritative #167 source and documented diagnosis, but must use a new
governed carrier. Do not manufacture a VOC-135 completion marker. Do not
reuse PR #1075 commits, branches, or review verdicts as implementation
authority. Cleanup after this replacement merges may close obsolete remote
automation branches/PRs/issues for VOC-127 and VOC-130 through VOC-135;
it must not delete or disturb unrelated VOC-128 work or user worktrees.
...
1. Pinning #164 (`863fc1f…`), #165 (`8ce2b77…`), or #166 (`f3d791…`) after
   #167 merged — forbidden by `VOC-136-D02`.
2. Silently retargeting VOC-132 / VOC-133 / VOC-134 / VOC-135 or redispatched
   T00 issues — forbidden by `VOC-136-D01` and `VOC-136-D10`.
...
4. Copying helpers after the unrestricted model, or treating a surviving
   non-distinct nested path as `absent` — forbidden by `VOC-136-D05`.
5. Fetching missing VOC-112 evidence commits from `run-app-checks.sh`, CI, or
   implementer pre-push — forbidden by `VOC-136-D06`.
6. Reusing PR #1051, PR #1056, PR #1065, PR #1070, or PR #1075 — forbidden by
   `VOC-136-D01`.
7. Re-implementing VOC-129, manufacturing a VOC-127 through VOC-135
   completion marker, or snapshotting the develop/main gap — forbidden by
   `VOC-136-D10` and `karsift-ai-infra#15`.
8. Retargeting VOC-112 JSON evidence — the VOC-130 exhaustion class,
   forbidden by `VOC-136-D14`.
9. Weakening VOC-112 provenance `local` mode fail-closed behavior, or
   ...
   VOC-131 exhaustion class, forbidden by `VOC-136-D14` and `VOC-136-D12`.
10. Fetching a missing capture commit before `pnpm test`, wrapping tests in
    ...
    the VOC-133 / VOC-134 exhaustion classes, forbidden by `VOC-136-D14` and
    `VOC-136-D16`.
11. Requiring a commit to contain its own SHA — the VOC-133-D12 defect,
    forbidden by `VOC-136-D12`.
12. Using a moving `develop` ref as the eight-path comparison authority, or
    implementing against unrelated/material movement of `develop` —
    forbidden by `VOC-136-D15`.
13. Validating #167 bytes only against a machine-specific `/tmp` checkout —
    forbidden by `VOC-136-D11`.
14. Scanning only one prohibited filename, or excluding
    `tooling/governance/tests/**` from the scan — the VOC-135 exhaustion
    class, forbidden by `VOC-136-D16`.
15. Committing contiguous forbidden assertion literals that make the
    tracked regression self-fail — the VOC-135 attempt-1 class, forbidden
    by `VOC-136-D16`.
16. Treating PR #1075 attempt-2 PASS WITH NON-BLOCKING FINDINGS as
    sufficient while the exclusion remains — forbidden by `VOC-136-D16`.
17. Erasing unique develop commits — still forbidden by VOC-127-D02,
    preserved in the #164/#165/#166/#167 fixture.
18. Treating a missing task-completion helper as a successful no-op after
    this repair — the restore must make the helper present; tests must fail
    if it can still disappear before use.
19. Printing App tokens, private keys, or secret values, or copying full CI
    logs / raw provider responses into evidence — forbidden.
20. Cleaning unrelated VOC-128 work or user worktrees — forbidden by
    `VOC-136-D17`.

## Contradictions and open questions

1. **Live caller workflow edit (`VOC-136-DEP-07`):** required behavior is
   ...
   `release.yml@main`.
2. **Live pin vs issue wording:** live tracked `PINNED_SHA.txt` at drafting
   equals #164 (`863fc1f…`) because VOC-134 and VOC-135 never merged. This is
   not an ambiguity about the pin target: this package pins to #167
   (`b263c0c…`) either way, and that merge already contains the #166 files.
   Implementation must not leave the pin at #164 or #166.
3. **Caller fixture README vs infrastructure README (`VOC-136-DEP-12`):** they
   ...
   SHA-256 is recorded in `VOC-136-D11` as the documentation-source hash.
...
6. **VOC-135 exhausted carrier:** VOC-135 remains an adopted #167 package
   whose implementation exhausted. This replacement consumes #167 under new
   authority rather than editing VOC-135 in place or redispatched #1073. Do
   not treat attempt-2 PASS WITH NON-BLOCKING FINDINGS as a reusable verdict.
7. **PR #1075 and earlier unmerged carriers:** the pin/restore portion of
   ...
   capture-commit fetch helpers, import-time fetch, hydrate helpers, and
   `SCAN_EXCLUDE_PREFIXES` weakenings cannot ride along.
8. **Drafting-time SHA-256 values (`VOC-136-D11`):** hashes above were
   recorded for VOC-135-D11 from an untracked nested checkout at `b263c0c…`
   and are reused because #167 has not moved. Implementation must reconfirm
   them from a fetched exact merge. If they differ, record the fetched hashes
   in `t00-evidence.md` and use those as the committed expected values; do
   not silently keep a stale drafting-time hash.
...
10. **Protected comparison anchor vs implementation PR base (`VOC-136-D15`):**
    expected protected comparison anchor is
    `b9e74fc2db4691c48c637639b265d527de9f4505`. Issue-creation `develop` is
    `5044d62f6f35069412e8ffcf80a0682e7847d1c1`. If live `develop` has
    unrelated/material movement at implementation dispatch, fail closed. Do
    not retarget the protected comparison anchor. Record the implementation
    PR base separately.
11. **VOC-129 promotion timing:** hosted `release.yml@main` / `implement.yml@main`
    ...
    implementation diff.
12. **VOC-128 and user worktrees:** issue #1076 requires preserving unrelated
    VOC-128 work and all user worktrees during cleanup. This package does not
    inspect or modify those surfaces. Cleanup is limited to obsolete remote
    automation branches/PRs/issues for the superseded pin carriers named in
    `VOC-136-D10`.
