...
- Prerequisites: confirm live `PINNED_SHA.txt` still equals
  ...
  run `33079499176`. Confirm VOC-133-T00 (#1063 / PR #1065) remains exhausted
  at attempt-1 head `e88fbda6274488bed898877151bbcc1714a45450` and attempt-2
  head `70930cf0572f73f254429a240c5328640846e0a9` and unmerged. Confirm the
  two named VOC-112 fixtures still contain `subject_revision`
  ...
  has no `VOC112_CAPTURE_PROVENANCE_MODE` wrapper and no capture-commit fetch
  helper. Confirm live `pipeline.yml` still dispatches `implement.yml@main`
  and `release.yml@main`, still exposes `reconcile-production-change`, and
  still has at most 25 `workflow_dispatch` inputs.
- Resolve current `develop` to a 40-character SHA **before any in-scope
  edit**. Expected SHA is `95a779f9e62090f856ed03f389e7ac1d901aaa14`. If live
  `develop` equals that SHA, record it as the immutable carrier base. If it
  differs, fail closed until the newly observed exact SHA is written into the
  regression and `t00-evidence.md` as the pre-edit constant. Do not proceed
...
- No bootstrap exception. VOC-124 already published
  ...
  attempt `1` on a new VOC-134 carrier from current `develop`. Do not treat
  an untracked local `karsift-ai-infra/` checkout as this repository's
  tracked tree. Do not reuse PR #1051, PR #1056, or PR #1065. Do not
  redispatch VOC-132-T00 (#1059) or VOC-133-T00 (#1063).
...
- Do not re-implement VOC-129 through VOC-133. Do not manufacture completion
  ...
  VOC-112 provenance test, or `package.json`. Do not add a capture-commit
  fetch helper, provenance-mode wrapper, or evidence-stamping helper. Do not
  require a commit to contain its own SHA.

## File reconciliation and implementation sequence

### T00 — Pin exact infra #166 with feasible exact-SHA evidence and a complete VOC-112 no-change boundary
...
| `tooling/governance/fixtures/karsift-ai-infra/README.md` | modify current-state pin paragraph | Record VOC-134-T00 advance to #166; do not rewrite historical pin sentences as if they never happened; do not replace this caller-owned README with the infrastructure repository README |
| `tooling/governance/tests/` | create/extend | Exact pin equality; inequality with #164 and #165 pins; identify and converge restore-before-helper coverage; helper-copy-before-model and nested-checkout classifier coverage including non-directory; recorded SHA-256 of mirrored #166 files; complete five-path VOC-112 no-change boundary vs immutable carrier-base SHA (fail, do not skip, if the commit or path cannot resolve); `package.json` carrier-base identity; feasible exact-revision evidence contract; update VOC-129 and other live pin assertions that currently hard-code `863fc1f…` |
...
| `.github/workflows/pipeline.yml` | modify only if `VOC-134-DEP-07` proves required | Expected no-op; caller already uses `implement.yml@main` and `release.yml@main` |
| `specs/changes/VOC-134-.../t00-evidence.md` | update | Record pin SHA, restore coverage, nested-checkout coverage, complete VOC-112 boundary vs immutable carrier-base SHA, `package.json` identity, hashes, feasible exact-head binding contract, final infra merge, validation commands, and VOC-129 through VOC-134 promotion handoff. Do not write the live implementation-head SHA into this file as a self-referential required value |

Ordered steps:

1. Resolve current `develop` to a 40-character SHA before any in-scope edit.
   Expected: `95a779f9e62090f856ed03f389e7ac1d901aaa14`. Record that SHA as
   the immutable carrier-base constant. If live `develop` differs, fail closed
   until the newly observed exact SHA is written into tests and
   `t00-evidence.md` before any other edit.
2. From current `develop`, create a new VOC-134 implementation branch. Do not
   reuse PR #1051, PR #1056, or PR #1065. Do not redispatch VOC-132-T00
   (#1059) or VOC-133-T00 (#1063). Do not re-check out or rewrite VOC-129 PR
   #1046.
3. Fetch KARSIFT/karsift-ai-infra at exact
   ...
   files; reconfirm against `VOC-134-D11` drafting-time hashes; commit the
...
4. Advance every live caller pin assertion that currently equals `863fc1f…`
   ...
   helper.
5. Add the complete VOC-112 no-change-boundary regression against the
   immutable carrier-base SHA for all five named paths. Add the `package.json`
   identity assertion against the same SHA. Confirm none of those paths is
   staged. Confirm `local` mode fail-closed text remains in the provenance
   test. Confirm no capture-commit fetch helper or provenance-mode wrapper
   exists. Confirm `t00-evidence.md` names the carrier base and the final
...
6. Update current-state fixture README/comments. Record evidence in
   `t00-evidence.md`. This package's caller PR `Closes` only its own VOC-134
   task issue.
7. After the exact reviewed caller PR merges, ordinary release evaluation (or
   ...
   VOC-129 through VOC-133 completion marker.
...
`python3 -m unittest tooling.governance.tests.test_voc134_complete_voc112_boundary`
or a recorded-hash module, or a `package.json` identity module), record the
exact commands in `t00-evidence.md` and run them in addition to the suite
above. Do not treat a missing suite as a pass. Do not run a command that
clones karsift-ai-infra into `/tmp` as a required check.
...
- all five VOC-112 no-change paths and `package.json` are absent from the
  diff against the immutable carrier-base SHA; JSON fixtures still carry
  `subject_revision` `f9d11e232a07c7d7a9c433d02c9267912543ba10`; the
  provenance test still fail-closes `local` mode on a missing capture commit
  in a full checkout; no capture-commit fetch helper or provenance-mode
  wrapper exists;
...
- VOC-113 through VOC-133 isolation, recovery, App-token split, lease,
  retry limits, Cursor Composer/Grok roles, unchanged `roles.yml`, and
  non-closing source PR remain;
- PR #1051, PR #1056, and PR #1065 were not reused; VOC-132-T00 (#1059) and
  VOC-133-T00 (#1063) were not redispatched; no snapshot-gap task and no
  bootstrap exception were used; no VOC-129 through VOC-133 completion marker
  was manufactured;
...
- **Rollback trigger:** pin not equal to `f3d791…`; restore missing in
  ...
  or `package.json` rewritten; capture-commit fetch helper or provenance-mode
  wrapper added; evidence mutated at test time; self-referential exact-head
  SHA required; PR #1051, PR #1056, or PR #1065 reused; VOC-132-T00 or
  VOC-133-T00 redispatched; `/tmp`-only byte comparison; false-revert
  evidence; or a snapshot-gap commit.
- **Rollback mechanism:** Revert the caller fixture/test/doc changes to the
  ...
  #166. Do not restore the VOC-112 JSON retargets from #1051, the
  provenance-test weakening from #1056, or the `package.json` /
  evidence-stamp changes from #1065.
- **Last-known-good reference:** caller `develop` before this package's merge,
  ...
  provenance-test fail-closed `local` mode, and unmodified `package.json`.
  Expected carrier-base SHA at drafting is
  `95a779f9e62090f856ed03f389e7ac1d901aaa14`. Infrastructure last known good
  for the consumed contract is #166 merge
