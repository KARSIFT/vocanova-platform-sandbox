...
This package intentionally defaults to **one task** because issue #1066 is one
caller replacement outcome: consume exact infrastructure #166 from current
`develop` with a complete VOC-112 no-change boundary, unchanged
`package.json`, feasible exact-revision evidence, restore shared policy after
caller checkout in both release jobs, land the #166 nested-checkout
helper-lifetime contract, and let ordinary release complete VOC-129,
exhausted VOC-130, exhausted VOC-131, superseded VOC-132, exhausted VOC-133,
and this blocker. Repository count, workflow-versus-tests-versus-docs,
fixture/pin work, the VOC-112 no-change boundary, package.json identity,
feasible evidence, and release reconciliation are not split reasons. VOC-129
/ VOC-130 / VOC-131 / VOC-132 / VOC-133 promotion and closure are evidence of
this outcome, not additional VOC-134 tasks and not a further attempt on those
...
docs, complete VOC-112 no-change boundary, package.json identity, hash-based
#166 byte identity, feasible exact-revision evidence, and evidence land in
this repository under the same task. No bootstrap exception: VOC-124 already
enables nested workflow-file publication; T00's first run is attempt `1` on a
new VOC-134 carrier from current `develop`. Do not reuse PR #1051, PR #1056,
or PR #1065. Do not redispatch VOC-132-T00 (#1059) or VOC-133-T00 (#1063).

This is not a "promote current develop to main" package and not a snapshot of
the develop/main gap (`karsift-ai-infra#15`).

## VOC-134-T00 — Pin exact infra #166 from current develop with feasible exact-SHA evidence and a complete VOC-112 no-change boundary

- Requirement source: issue #1066; `VOC-134-D00` through `VOC-134-D15`
- Acceptance criteria: `VOC-134-AC-00` through `VOC-134-AC-09`
- Tests: `VOC-134-TEST-00` through `VOC-134-TEST-12`
- Evidence: `VOC-134-EV-00` (`t00-evidence.md` in this package directory)
- Status: pending

### Required work

1. Record the issue #1066 evidence in `t00-evidence.md` (exhausted VOC-133-T00
   #1063 / PR #1065, attempt-1 head `e88fbda6274488bed898877151bbcc1714a45450`
   FAIL comment 5441321518, attempt-2 head
   `70930cf0572f73f254429a240c5328640846e0a9` FAIL comment 5441659390,
   supervisor comment 5441214837, VOC-133-D12/TEST-10 self-referential SHA
   defect, VOC-132 run `33079499176` / job `98543693163`, bounded `cp: cannot
   stat karsift-ai-infra/config/run-app-checks.sh`, no VOC-132 caller
   branch/PR/completion marker, prohibition on redispatched VOC-132-T00 #1059
   and VOC-133-T00 #1063, exhausted VOC-130 task #1049 / PR #1051, exhausted
   VOC-131 PR #1056, VOC-129 caller PR #1046 at
   `429d8c6d49303148ca1cc14dba5f6768a7863346`, original release run
   ...
   `863fc1f35b1d35e4981a59166b0e939be1a2b681`, expected immutable carrier
   base `95a779f9e62090f856ed03f389e7ac1d901aaa14`, VOC-112
...
2. Resolve current `develop` to a 40-character SHA **before any in-scope
   edit**. Expected SHA is `95a779f9e62090f856ed03f389e7ac1d901aaa14`. If
   live `develop` equals that SHA, record it as the immutable carrier base.
   If it differs, fail closed: record the newly observed exact SHA as the
   pre-edit immutable constant in the regression and `t00-evidence.md` before
   touching any other in-scope file. Do not treat a moving branch name as
   authority after that constant is selected.
3. From current `develop`, create a new VOC-134 implementation branch. Do not
   reuse, merge, cherry-pick, modify, or compose PR #1051, PR #1056, PR #1065,
   or their branches. Do not redispatch VOC-132-T00 (#1059) or VOC-133-T00
   (#1063). Do not re-check out or rewrite VOC-129 PR #1046.
...
6. Add a deterministic regression bound to the immutable carrier-base SHA
   that fails if any of the five VOC-112 no-change paths differs from that
   SHA or appears in the implementation diff:
   ...
   The test must require the exact commit object and all five paths to
   resolve and must fail—not skip—if proof cannot run. Confirm JSON
   `subject_revision` remains `f9d11e232a07c7d7a9c433d02c9267912543ba10`.
   Confirm the provenance test still fail-closes `local` mode on a missing
   capture commit in a full checkout. Do not retarget, recapture, or weaken
   those files. Do not use a moving branch ref as the authority after the
   SHA is selected.
7. Assert `package.json` is byte-identical to the same immutable carrier-base
   SHA. Do not add a capture-commit fetch helper, provenance-mode wrapper,
   evidence-stamping helper, test-time evidence mutation, or any invocation
   that masks default `local` fail-closed behavior. Do not modify evidence at
   test or runtime.
8. Preserve #164 contracts: missing-`develop` recovery, branch exact-SHA
   ...
   values.
9. Update current-state fixture README/comments so they name pin `f3d791…`,
   ...
   VOC-127 / VOC-129 / VOC-130 / VOC-131 / VOC-132 / VOC-133 audit records.
10. Live `.github/workflows/pipeline.yml` changes only if comparing the #166
...
11. This package's caller PR `Closes` only its own VOC-134 task issue. Do not
    re-implement VOC-129, VOC-130, VOC-131, VOC-132, or VOC-133. Do not
    manufacture a completion marker for those packages. Do not snapshot the
    current develop/main gap.
12. Record in `t00-evidence.md` the immutable carrier-base SHA, the final
    ...
    equality results, the feasible exact-head binding contract (App-authored
    independent-review comment/check; merge-gate rejects mismatch;
    post-merge audit may name reviewed head and merge SHA), and — after
    merge — the ordinary release/promotion/sync/closure evidence for VOC-129
    through VOC-133 and this package. Evidence must not require a commit to
    contain its own SHA. Evidence must not claim a protected-path revert
    unless that path is absent from the diff. Hosted `implement.yml@main` /
    `release.yml@main` may already contain the #166 / #165 repairs before
    this pin merges; `reconcile-release` for VOC-129 may succeed
...
13. Run applicable validation and record results in `t00-evidence.md`:
    ...
    - targeted foundation tests if those files change (`voc097`, `voc104`,
      `voc108`, exact #166 pin/hash tests, immutable carrier-base VOC-112
      boundary tests, and the `package.json` carrier-base-identity
      assertion);
    - exact pin SHA `f3d79177bf8a9abe0dae550f39502165d494c576`;
    - recorded SHA-256 identity of the mirrored #166 files;
    - five-path VOC-112 and `package.json` equality against the immutable
      carrier-base SHA and diff absence;
    - `git diff --check`;
...
- Redispatching VOC-132-T00 (#1059) or VOC-133-T00 (#1063).
- Reusing, merging, cherry-picking, modifying, or composing PR #1051,
  PR #1056, or PR #1065.
- Re-implementing VOC-129, VOC-130, VOC-131, VOC-132, or VOC-133, or
  manufacturing a completion marker for those packages.
- Changing any of the five VOC-112 no-change paths, including weakening
  ...
  this pin.
- Changing `package.json` or adding a capture-commit fetch helper,
  provenance-mode wrapper, evidence-stamping helper, or test-time evidence
  mutation.
- Requiring a commit to contain its own SHA.
- Using a moving branch ref as the no-change authority after the immutable
  carrier-base SHA is selected.
...
- Operator-owned live evidence contracts: acceptance is deterministic tests
  plus exact-SHA review and the recorded VOC-129 through VOC-134 promotion
  handoff.
...
- This package intentionally has one task because no concrete split boundary
  ...
  complete VOC-112 no-change boundary, package.json identity, hash-based byte
  identity, feasible exact-revision evidence, preserved #164 contracts, docs,
  and VOC-129 through VOC-134 promotion handoff are one repair outcome.
...
- VOC-129 promotion may proceed through hosted `release.yml@main` as soon as
  ...
  VOC-134 task to wait on that event.
