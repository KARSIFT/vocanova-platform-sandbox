...
workspace root, with a complete VOC-112 no-change boundary. It does not
...
- `VOC-132-R00`: **High correctness risk** if the caller pin remains
  `863fc1f…` while live `release.yml@main` is already #165. Local suites would
  then assert a stale restore-less fixture. Mitigation: `VOC-132-D02`,
  `VOC-132-AC-00`.
- `VOC-132-R01`: **High release-safety risk** if `identify` restores shared
  policy but `converge` does not, repeating the latent defect that skipped
  promotion in run `33066533397`. Mitigation: `VOC-132-D03`, `VOC-132-AC-02`.
- `VOC-132-R02`: **High correctness risk** if restore uses a mutable ref
  ...
  caller ref. Mitigation: `VOC-132-D03`, `VOC-132-TEST-04`.
- `VOC-132-R03`: **High data-loss risk** if pinning #165 accidentally drops
  #164 unique-develop fail-closed sync or missing-`develop` recovery.
  Mitigation: `VOC-132-D04`, `VOC-132-AC-03`.
- `VOC-132-R04`: **High delivery risk** if this package snapshots the current
  develop/main gap (`karsift-ai-infra#15`), reuses PR #1051 or PR #1056, or
  re-implements VOC-129 / VOC-130 / VOC-131. Mitigation: `VOC-132-D01`,
  `VOC-132-D08`, `VOC-132-AC-06`.
- `VOC-132-R05`: **High correctness / review-safety risk** if the
  ...
  `c11454e717a6d778143de1f2023acc4480305845`. Mitigation: `VOC-132-D10`,
  `VOC-132-AC-05`, `VOC-132-TEST-08`.
- `VOC-132-R06`: **High correctness / review-safety risk** if the
  ...
  `VOC-132-D10`, `VOC-132-AC-05`.
- `VOC-132-R07`: **High coverage risk** if the identity regression again
  ...
  Mitigation: `VOC-132-D10`, `VOC-132-TEST-08`.
- `VOC-132-R08`: **High correctness risk** if #165 byte identity depends on a
  machine-specific `/tmp` checkout that CI or another machine does not have.
  Mitigation: `VOC-132-D11`, `VOC-132-TEST-01`.
- `VOC-132-R09`: **High review-safety risk** if `t00-evidence.md` claims a
  protected-path revert that the exact reviewed tree does not contain, as on
  #1056. Mitigation: `VOC-132-D12`, `VOC-132-TEST-09`.
- `VOC-132-R10`: **Medium coverage risk** if caller tests advance the pin but
  do not assert restore-before-helper ordering. Mitigation: `VOC-132-D05`,
  `VOC-132-TEST-02`, `VOC-132-TEST-03`.
- `VOC-132-R11`: **Low application-runtime release risk** because ordinary
...
- Protected surfaces: caller `tooling/governance/` fixtures and tests,
  possibly live `.github/workflows/*` if `VOC-132-DEP-07` proves a caller
...
- `VOC-132-DEP-00` through `VOC-132-DEP-09`: see `change.yaml`.
- `VOC-132-EV-00`: T00 evidence — pin SHA, restore coverage, complete VOC-112
  no-change boundary, hash-based #165 identity, preserved #164 contracts,
  validation commands, exact reviewed head, and VOC-129 / VOC-130 / VOC-131 /
