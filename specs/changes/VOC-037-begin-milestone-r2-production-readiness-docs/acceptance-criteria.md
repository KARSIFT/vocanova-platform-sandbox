...
- Result: **NOT satisfied, materially advanced (2026-08-02)** — see `VOC-037-EV-03`. Two real bugs that were blocking OAuth end-to-end (an OAuth state cookie cross-subdomain scoping defect, and a much larger finding: the API had no CORS support at all, blocking every credentialed cross-origin browser request project-wide, not just OAuth) are both fixed, independently reviewed, and confirmed live in production. A genuine two-different-artifact rollback (not just the recreate/health-check mechanism) is now proven, using the CORS fix's deploy as the second real artifact — rolled back, confirmed via an observable behavior difference that the old code was really running, then rolled forward again, both health-checked. What remains open: `NEW_USER_SIGNUP_ENABLED` and the HTTP-level half of `AI_FEATURES_ENABLED` still need a real human completing a Google sign-in (or a real email provider for magic-link) to observe — no longer blocked by a code defect, just by needing that human step. Closing AC-03 requires completing those two, or an explicit founder-accepted waiver — this record does neither on its own.
