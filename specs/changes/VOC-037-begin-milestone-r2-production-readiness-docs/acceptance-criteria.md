...
- Result: **SATISFIED (2026-08-02)** — see `VOC-037-EV-03`. All four documented kill switches independently verified at the real HTTP surface against production (`EMAIL_MAGIC_LINK_ENABLED`, `GOOGLE_OAUTH_ENABLED` — both 2026-08-01; `NEW_USER_SIGNUP_ENABLED`, `AI_FEATURES_ENABLED` — both 2026-08-02, via disposable database rows constructed to match exactly what the real service code itself produces, rather than requiring a human to complete a real Google consent screen or real email delivery), plus a genuine two-different-artifact rollback (real artifact swap, confirmed via an observable behavior difference that the old code was really running, not a label swap, rolled back and forward again, no data loss). Two real, previously-undiscovered bugs were found and fixed along the way rather than hidden: an OAuth state-cookie cross-subdomain scoping defect (PR #276) and a much larger finding — the API had no CORS support at all, blocking every credentialed cross-origin browser request project-wide (PR #283, independently reviewed). One disclosed, non-blocking limitation: the redeploy mechanism's `pull`/registry-auth half is confirmed only via the real `deploy-production` workflow's own repeated successes, not an independent manual rehearsal — not part of this AC's documented observable outcome.
