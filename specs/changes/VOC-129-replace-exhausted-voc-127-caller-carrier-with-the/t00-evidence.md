...
- `tooling/governance/fixtures/karsift-ai-infra/**` mirrored from infra #164,
  including `release.yml`, `reconcile-production-change.yml`,
  `release-checkout-ref-runner.py`, `branch_sync.py`, `branch-sync-runner.py`,
  `verify-production-merge-guard.sh`, `production_merge_guard.py`,
  `production-merge-guard-runner.py`, `merge-gate.yml`, template `pipeline.yml`,
  and checkout-ref / branch-sync / release-policy tests.
- `tooling/governance/fixtures/karsift-ai-infra/PINNED_SHA.txt` set to
  `863fc1f35b1d35e4981a59166b0e939be1a2b681`.
- `.github/workflows/pipeline.yml` exposes `reconcile-production-change`,
  forwards `issue_number` as authority, makes `release` need
  `[merge-gate, reconcile-production-change]`, and makes `auto-advance` wait on
  successful production-change reconcile. Preserved live `live_evidence_mode`.
- `scripts/foundation/voc111-deploy-staging-paths.test.mjs` extended for
  tree-equivalent empty diff and specs-only paths.
- Caller governance and foundation pin literals advanced to `863fc1f…`.
- `tooling/governance/tests/test_voc129_caller_replacement.py` added.
- Current-state docs updated in `AGENTS.md`, `docs/operations/10-development-workflow.md`,
...
- No `deploy-staging.yml` job-level skip added: VOC-111 `on.push.paths` already
  excludes tree-equivalent merge-commit fast-forwards with no allowlisted path
  changes.
...
| Pin must not equal | `a9df74a63976d5239b84151fd01310835c999e7c` or `60afda3a44fd06b8c00b219771de7112f1aded6e` |
| `PINNED_SHA.txt` | `863fc1f35b1d35e4981a59166b0e939be1a2b681` |
| Bootstrap used? | **no** |
| #1041 published? | **no** |
...
| caller governance unittest suite | pass (221 tests) |
| fixture unittest suite | pass (378 tests) |
| `voc111-deploy-staging-paths.test.mjs` | pass (10 tests) |
...
- `VOC-129-AC-00` / `VOC-129-EV-00` — implemented; pin and mirrored #164 files verified
- `VOC-129-AC-01` / `VOC-129-EV-00` — implemented; checkout-ref tests execute in fixture
- `VOC-129-AC-02` / `VOC-129-EV-00` — implemented; live `reconcile-production-change` dispatch and 25-input bound
- `VOC-129-AC-03` / `VOC-129-EV-00` — implemented; fixture `release.yml` binds merge SHA, no `CHECKED_HEAD_SHA` restore
- `VOC-129-AC-04` / `VOC-129-EV-00` — implemented; VOC-111 path selection retained; empty/specs-only paths excluded
- `VOC-129-AC-05` / `VOC-129-EV-00` — implemented on carrier; post-promotion #1041/#1039/#1035 closes pending promotion
- `VOC-129-AC-06` / `VOC-129-EV-00` — implemented; docs updated; `roles.yml` unchanged; no OpenAI route
