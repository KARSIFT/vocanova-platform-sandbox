# Pinned karsift-ai-infra contract fixtures (VOC-080-T05, VOC-097-T03, VOC-102-T00, VOC-104, VOC-106, VOC-108, VOC-115, VOC-117, VOC-121, VOC-122, VOC-123, VOC-126, VOC-129, VOC-133)
...
VOC-097-, VOC-102-, VOC-104-, VOC-106-, VOC-108-, VOC-115-, VOC-121-, VOC-122-, VOC-123-, VOC-126-, VOC-129-, or VOC-133-related infra contracts change and
...
byte-for-byte copies from that merge's reviewed tree. VOC-133-T00 advances it to
infrastructure PR #166 merge
`f3d79177bf8a9abe0dae550f39502165d494c576`; the mirrored
`.github/workflows/implement.yml`, `.github/workflows/release.yml`,
`config/implementer_nested_checkout.py`, and the paired policy tests are
byte-for-byte copies from that merge's reviewed tree. Both `identify` and
`converge` now contain `Restore shared lifecycle policy after caller checkout`
after `Checkout caller release state` and before
`task-completion-runner.py` (`validate-task` / `validate-roster`), using
`repository: ${{ job.workflow_repository }}`, `ref: ${{ job.workflow_sha }}`,
`path: karsift-ai-infra`, and `persist-credentials: false`. Fixture
`implement.yml` now contains `Preserve post-implementer lifecycle helpers`
before the unrestricted model and classifies nested checkout from the preserved
`/tmp` helper after the model instead of the possibly deleted nested tree.
