...
python_min="$(read_identity_field python_min)"
python3 -c 'import sys; raise SystemExit(sys.version_info < (3, 12))' \
  || die "python $python_min or newer is required by the reviewed lock"
...
PIP_DISABLE_PIP_VERSION_CHECK=1 PIP_NO_INPUT=1 \
  "$VENV_DIR/bin/python" -m pip install \
    --require-hashes \
    --only-binary=:all: \
    -r "$REQUIREMENTS_LOCK" >/dev/null

bash "$GRAPHIFY_HOME/check" >/dev/null

echo "graphify-pilot setup: repository-local locked runtime ready at $VENV_DIR"
