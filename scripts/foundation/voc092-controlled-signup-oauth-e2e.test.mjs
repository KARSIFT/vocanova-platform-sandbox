...
test("VOC-092-TEST-12: harness source files and package script are committed", () => {
  ...
  assert.match(
    ...
    /test:controlled-signup-oauth-e2e/,
    "pnpm test must invoke the dedicated harness script",
  );
...
test("VOC-092-TEST-07: harness fixtures use only synthetic.vocanova.invalid identities", () => {
  ...
  for (const email of emailMatches) {
    assert.match(
      email,
      /@synthetic\.vocanova\.invalid$/i,
      `harness email must stay on the synthetic domain: ${email}`,
    );
  }
...
test("VOC-092-TEST-15: harness sources exclude staging/production hosts and secret env reads", () => {
  ...
  assert.match(
    harnessSources,
    /docker not on PATH/,
    "harness must fail closed when Docker is unavailable instead of connecting elsewhere",
  );
});
