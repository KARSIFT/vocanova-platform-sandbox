...
SELECT jsonb_build_object(
 ...
 'settings', COALESCE((SELECT jsonb_build_object('timezone', timezone, 'dailyReviewTarget', daily_review_target, 'reviewIntervalPreset', review_interval_preset, 'notificationsEnabled', notifications_enabled, 'marketingEmailsEnabled', marketing_emails_enabled, 'appLanguage', app_language, 'createdAt', created_at, 'updatedAt', updated_at) FROM user_settings WHERE user_id = $1), '{}'::jsonb),
 ...
 'savedWords', COALESCE((SELECT jsonb_agg(jsonb_build_object('id', id, 'meaningId', meaning_id, 'status', status, 'source', source, 'reviewStep', review_step, 'nextReviewAt', next_review_at, 'lastReviewedAt', last_reviewed_at, 'lastResult', last_result, 'lastRating', last_rating, 'consecutiveCorrectCount', consecutive_correct_count, 'consecutiveIncorrectCount', consecutive_incorrect_count, 'totalReviewCount', total_review_count, 'correctReviewCount', correct_review_count, 'addedAt', added_at, 'masteredAt', mastered_at, 'ignoredAt', ignored_at, 'createdAt', created_at, 'updatedAt', updated_at) ORDER BY added_at) FROM user_words WHERE user_id = $1 AND deleted_at IS NULL), '[]'::jsonb),
