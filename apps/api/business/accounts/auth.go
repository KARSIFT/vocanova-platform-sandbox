...
type Repository interface {
	...
	// CleanupExpiredEmailChangeLinks removes at most limit transient email-change
	// credentials after expiry, consumption, or revocation. It is only called by
	// the production lightweight-cleanup loop. The bounded claim makes concurrent
	// API replicas safe without a queue.
	CleanupExpiredEmailChangeLinks(ctx context.Context, before time.Time, limit int) (int64, error)
