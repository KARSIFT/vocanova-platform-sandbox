...
func TestPaginationHonorsDocumentedMaximum(t *testing.T) {
	...
	page, err := repo.ListSavedWords(t.Context(), ListSavedWordsRequest{UserID: userID, Limit: 100})
	require.NoError(t, err)
	require.Equal(t, 50, len(page.Items))
	require.NotEmpty(t, page.NextCursor)
	last, err := repo.ListSavedWords(t.Context(), ListSavedWordsRequest{UserID: userID, Limit: 100, AfterCursor: page.NextCursor})
	require.NoError(t, err)
	...
	defaultPage, err := repo.ListSavedWords(t.Context(), ListSavedWordsRequest{UserID: userID})
	require.NoError(t, err)
